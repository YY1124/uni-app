<template>
	<view class="preach-user-service">
		<view class="top">
			<text class="text">
		...
		<view class="content-box" @click="Go(item)" v-for="item in item" :key="item.Id">
			...
			<view v-if="(item.Status)==1" style="background:#FFA200;" class="state">
				未开始
			</view>
			<view v-if="(item.Status)==2" style="background:#21C499;" class="state">
				进行中
			</view>
			<view v-if="(item.Status)==3" style="background:#A3A3A3;" class="state">
				已结束
			</view>
		</view>
	</view>
